class_name GrablessTracker extends Node3D

@export var hand_detection_area: Area3D ## Area that should only detect hand area collision shapes.

# Default CSV column headers. Linear+angular velocity and position.
const headers: String = \
						#"lv_x, lv_y, lv_z, av_x, av_y, av_z, pos_x, pos_y, pos_z, rot_x, rot_y, rot_z," + \
...
func _ready() -> void:
	...
	hand_detection_area.area_entered.connect(_on_hand_entered);
	hand_detection_area.area_exited.connect(_on_hand_exited);
	initialize();
...
func record_metrics():
	...
	if picked_up: #and (rb.linear_velocity.length() or rb.angular_velocity.length()):
		...
		data += \
			#str(rb.linear_velocity.x) + ", " + str(rb.linear_velocity.y) + ", " + str(rb.linear_velocity.z) + ", "\
...
func save_data(filename: String = ""):
	if filename == "":
		filename = str(action_name) + "_" + str(idx) + ".csv";
...
func _on_recording_toggle():
	...
	print("Recording " + str(action_name) + " metrics: " + str(Globals.is_recording));

func _on_hand_exited(_area: Area3D) -> void:
	#await get_tree().create_timer(0.1).timeout;
	#if rb.is_picked_up(): return;
	picked_up = false;
	if Globals.is_recording:
		save_data();
	Globals.send_socket_data("DROP " + action_name)
	Globals.flag = "";

func _on_hand_entered(_area: Area3D) -> void:
	print("hand detected")
	if picked_up: return
	Globals.send_socket_data("PICKUP " + action_name)
	picked_up = true;
	Globals.flag = action_name;
